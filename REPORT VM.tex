...
Figure~\ref{fig:miss-curves} shows page faults vs. frames $F\in\{2..8\}$ for all policies.

\begin{figure}[H]
\centering
\includegraphics[width=.8\linewidth]{figs/miss_curves_vm.png}
\caption{Page faults vs. frames $F\in\{2..8\}$ under the course workload; FIFO may exhibit Belady's anomaly under adversarial traces, while LRU is monotonic (stack property).}
\label{fig:miss-curves}
\end{figure}
